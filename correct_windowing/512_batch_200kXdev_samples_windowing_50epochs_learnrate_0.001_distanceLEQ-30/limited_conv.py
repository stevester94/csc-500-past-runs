...
EXPERIMENT_NAME = "512_batch_200kXdev_samples_windowing_50epochs_learnrate_0.001_distanceLEQ-30"
...
def get_shuffled_and_windowed_from_pregen_ds():
    ...
    train_ds = train_ds.filter(lambda x: x["distance_feet"] <= 30)
    val_ds = val_ds.filter(lambda x: x["distance_feet"] <= 30)
    test_ds = test_ds.filter(lambda x: x["distance_feet"] <= 30)
